...
function vis_slabs_hv(sl1, sl2, segh, segv)
    ...
    for i=1:length(sl1.slab_ids)-1
        sth = sl1.slab_ids(i);
        enh = sl1.slab_ids(i+1)-1;
        
        if(exist('segh', 'var') && exist('segv', 'var'))
            for j=1:length(sl2.slab_ids)-1
                stv = sl2.slab_ids(j);
                env = sl2.slab_ids(j+1)-1;
                I2(sth:enh,stv:env) = I2(sth:enh,stv:env) * segh(i)* segv(j) * 3;
            end
        else
            for j=1:length(sl2.slab_ids)-1
                stv = sl2.slab_ids(j);
                env = sl2.slab_ids(j+1)-1;
                I2(sth:enh,stv:env) = I2(sth:enh,stv:env) * i * j * 3;
            end
        end
    end
